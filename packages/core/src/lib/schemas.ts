...
// パッケージ境界をまたぐランタイムブリッジ。@zstate/core が（バージョン違い等で）
// 二重ロードされても同一シンボルを共有できるよう、グローバルシンボルレジストリ
// （Symbol.for）を使う。これで typedSetup(attach) ↔ getSchemas の橋が壊れない。
export const SCHEMAS_KEY: unique symbol = Symbol.for('@zstate/schemas');
